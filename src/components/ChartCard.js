...
function ChartCard() {
    ...
    const isLoading = useSelector(state => state.chartLoading);
    const date_range = useSelector(state => {
        if (!state.income) return null;

        const quarterlyReports = state.income['quarterlyReports'];
        if (!quarterlyReports) return null;

        return convertDates({
            start_date: quarterlyReports[0]['fiscalDateEnding'],
            end_date: quarterlyReports[quarterlyReports.length - 1]['fiscalDateEnding'],
        });
    });
                ...
                <Card bg="light" text="dark" style={{ position: 'relative', width: '80vw' }} className="mb-2">
                    ...
                    <Card.Body className="md:min-h-[500px] lg:min-h-[700px]">
                        ...
                        {isLoading ? (
                            ...
                            date_range ? <DataChart /> : <div className="flex justify-content-center text-5xl pt-10 pb-20">There is no data to display.</div>
                        )}
                    </Card.Body>
                </Card>
            ) : (
